...
/*   Updated: 2020/11/30 11:05:46 by yhadari          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_fc(void)
{
	...
	while (i < g_gt.win_len / 2)
	{
		...
		while (j < g_gt.win_wid)
		{
			ft_put_pixel(j, i + (g_gt.win_len / 2), ft_color(1));
			j++;
		}
		i++;
	}
...
void	ft_1map3d3(int i, float ystart, float dist, int color)
{
	...
	distfromstart = ystart + dist / 2 - g_gt.win_len / 2;
...
void	ft_map3d3(int i)
{
	...
	while (i < g_num_rays)
	{
		...
		dist = g_tile * ((g_gt.win_wid + g_gt.win_len) / 2) / g_ray[i].dis;
		ystart = round(g_gt.win_len / 2 - dist / 2);
		ystart = ystart < 0 ? 0 : ystart;
		yend = round(g_gt.win_len / 2 + dist / 2);
		yend = yend > g_gt.win_len ? g_gt.win_len : yend;
		if (g_ray[i].vertwall)
