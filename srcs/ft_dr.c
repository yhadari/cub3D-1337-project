...
/*   Updated: 2020/11/30 11:13:04 by yhadari          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_pixel(int x, int y, int color)
{
	...
	if (x >= 0 && y >= 0 && x < g_gt.win_wid && y < g_gt.win_len)
	{
...
int		ft_update(void)
{
	...
	i = 0;
	g_image_ptr = mlx_new_image(g_mlx_ptr, g_gt.win_wid, g_gt.win_len);
...
int		main(int argc, char **argv)
{
	...
	g_mlx_ptr = mlx_init();
	ft_readfile();
	g_win_ptr = mlx_new_window(g_mlx_ptr, g_gt.win_wid, g_gt.win_len, "cub3D");
